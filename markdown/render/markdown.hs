...
main = do
    posts <- glob "../posts/[0-9]*[0-9]"
    postHtml <- sequence (map postToText $ reverse . sort $ posts) >>= return . joinText
    indexNodes   <- readToNode "../The Scrapyard"
    --putStr $ T.unpack $ nToHtml $ addPosts postHtml indexNodes
    --putStr $ T.unpack $ nToHtml $ addPosts postHtml indexNodes
    putStr . T.unpack $ nToHtml indexNodes
    putStr . T.unpack $ makeTag "div" [("class", "h-feed")] $ T.append (makeTag "h3" [("class", "p-name")] $ T.pack "Miscellaneous Ramblings") postHtml
    where   postToText fn = withFile fn ReadMode parsePostToHtml
            addPosts posts (Node _ DOCUMENT ls) = Node Nothing DOCUMENT (ls ++ [(Node Nothing (HTML_BLOCK posts) [])])
            readToNode :: String -> IO Node
            readToNode fn = TIO.readFile fn >>= return . cmToNode

parsePostToHtml :: Handle -> IO T.Text
parsePostToHtml h = do
    title  <- TIO.hGetLine h
    date   <- TIO.hGetLine h
    author <- TIO.hGetLine h >>= return . head . nodeChildren . head . nodeChildren . cmToNode --Author link produces a PARAGRAPH in a DOCUMENT, and in there the LINK
    anchor <-     hGetLine h
    body   <- TIO.hGetContents h >>= return . cmToHtml
